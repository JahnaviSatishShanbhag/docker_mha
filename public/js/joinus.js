function validateTextbox() {
    ...
    var day = document.getElementById("category");
    var month = document.getElementById("subcategory");
    var msg;
    var namep = /[0-9]/;
    var phonep = /[a-z]/;
    ...
    if (name.value.trim() == " " || email.value.trim() == "" || phone.value.trim() == "" || address.value.trim() == "" || day.value.trim() == "" || month.value.trim() == "") {
...
function redirectPage() {
    ...
    if (check == true) {
        ...
        var day = document.getElementById("category").value;
        var month = document.getElementById("subcategory").value;
        console.log(day);
        fetch('http://localhost:5500/joinus', {
            headers: {
                'Content-type': 'application/json'
            },
            method: 'POST',
            body: JSON.stringify({ name: name, phone: phone, address: address, email: email, day: day, month: month })
        })
            .then((response) => {
                if (response.status == 200) {
                    location.href = "http://localhost:5500/ourpartners";
                }
